...
type FinalRole = 'student' | 'teacher' | 'researcher' | 'mentor' | null;
...
const OrbitingNetwork = () => {
  const nodes = [
    { size: 40, distance: 90, duration: 6, color: 'from-emerald-400 to-accent', icon: GraduationCap, delay: 0 },
    { size: 35, distance: 120, duration: 8, color: 'from-cyan-400 to-emerald-500', icon: Microscope, delay: 0.5 },
    { size: 38, distance: 150, duration: 10, color: 'from-accent to-cyan-400', icon: Brain, delay: 1 },
    { size: 33, distance: 105, duration: 7, color: 'from-emerald-500 to-accent', icon: Atom, delay: 1.5 },
  ];
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      ...
      <motion.div
        className="relative w-24 h-24 rounded-full bg-gradient-to-br from-emerald-400 via-accent to-cyan-400 flex items-center justify-center"
        animate={{
          boxShadow: [
...
const AnimatedVisual = () => {
  return (
    <div className="w-1/2 relative flex items-center justify-center px-8">
      ...
      />
      <motion.div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[250px] h-[250px] bg-accent/15 rounded-full blur-[100px]"
        animate={{
          scale: [1, 1.4, 1],
      ...
      <div className="relative z-10 w-full max-w-[500px]">
        ...
        >
          ...
          >
            ...
            <br />
            <motion.span
              className="inline-block bg-gradient-to-r from-cyan-400 via-accent to-emerald-400 bg-clip-text text-transparent"
              animate={{
        ...
        >
          {[
            ...
            { label: 'Institutions', value: '200+', color: 'accent', gradient: 'from-accent to-emerald-500' },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
...
export const SignupPage = () => {
  ...
  const [finalRole, setFinalRole] = useState<FinalRole>(null);
    ...
    try {
      ...
      const { error: profileError } = await supabase
        ...
        .insert({
          ...
          role: finalRole,
          institution:
            finalRole === 'researcher' || finalRole === 'mentor'
              ? institution || null
              : null,
        });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-forest-950 via-forest-900 to-forest-950 flex overflow-hidden relative">
      ...
      <div className="w-1/2 flex items-center justify-center px-8 relative z-10">
        ...
        >
          <motion.div 
            className="relative bg-gradient-to-br from-forest-800/60 via-forest-800/40 to-forest-900/60 backdrop-blur-2xl border border-accent/20 rounded-3xl p-7 shadow-[0_0_100px_rgba(16,185,129,0.15),0_20px_80px_rgba(0,0,0,0.6)]"
            whileHover={{
          ...
          >
            ...
            <motion.div 
              className="absolute top-0 left-0 w-12 h-12 border-l-2 border-t-2 border-accent/40 rounded-tl-3xl"
              animate={{
            ...
            />
            <motion.div 
              className="absolute bottom-0 right-0 w-12 h-12 border-r-2 border-b-2 border-accent/40 rounded-br-3xl"
              animate={{
                borderColor: ['rgba(16, 185, 129, 0.4)', 'rgba(16, 185, 129, 0.8)', 'rgba(16, 185, 129, 0.4)'],
              }}
            ...
            {/* Enhanced Branch Roles */}
            <AnimatePresence mode="wait">
              {primaryRole === 'academia' && (
                <motion.div
                  initial={{ opacity: 0, height: 0, y: -10 }}
                  animate={{ opacity: 1, height: 'auto', y: 0 }}
                  exit={{ opacity: 0, height: 0, y: -10 }}
                  transition={{ duration: 0.3, ease: 'easeOut' }}
                  className="flex gap-2.5 mb-3 overflow-hidden"
                >
                  <RoleCard
                    title="Student"
                    description="Learn & grow"
                    icon={GraduationCap}
                    selected={finalRole === 'student'}
                    onClick={() => setFinalRole('student')}
                  />
                  <RoleCard
                    title="Teacher"
                    description="Teach & guide"
                    icon={GraduationCap}
                    selected={finalRole === 'teacher'}
                    onClick={() => setFinalRole('teacher')}
                  />
                </motion.div>
              )}
              ...
              {primaryRole === 'research' && (
                ...
                >
                  <RoleCard
                    title="Researcher"
                    description="Analyze & publish"
                    icon={Microscope}
                    selected={finalRole === 'researcher'}
                    onClick={() => setFinalRole('researcher')}
                  />
                  <RoleCard
                    title="Mentor"
                    description="Guide researchers"
                    icon={Microscope}
                    selected={finalRole === 'mentor'}
                    onClick={() => setFinalRole('mentor')}
                  />
                </motion.div>
              )}
            </AnimatePresence>
            ...
            >
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="relative w-full mt-3 bg-gradient-to-r from-accent via-emerald-500 to-cyan-400 bg-[length:200%_100%] text-white font-bold rounded-xl py-3.5 shadow-[0_0_30px_rgba(16,185,129,0.4)] disabled:opacity-50 overflow-hidden group"
                animate={{
                  backgroundPosition: ['0% 50%', '100% 50%', '0% 50%'],
                }}
                transition={{
            ...
            >
              <Link
                to="/login"
                className="text-sm text-text-muted hover:text-accent flex items-center gap-2 transition-all duration-300 group"
              >
                <motion.div
                  animate={{ x: [0, -3, 0] }}
                  transition={{ duration: 1.5, repeat: Infinity }}
